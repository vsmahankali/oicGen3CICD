...
read -p "Enter name of directory where the lookups must be imported from: " DIR
...
IMPORT_URL="${INSTANCE_DESIGN_TIME_URL}/ic/api/integration/v1/lookups/archive?integrationInstance="
...
for id in $(jq -r ".${ARRAY_ATTRIBUTE}[].id" "$OUTPUT_FILE"); do
    ...
    if [[ "$status" == "CONFIGURED" ]]; then
            if [[ "$locked_flag" == "false" ]]; then
                ...
                FULL_IMPORT_URL="${IMPORT_URL}${OIC_INSTANCE}"
                FULL_IMPORT_URL="${FULL_IMPORT_URL//\{name\}/$name}"

                echo "Full URL: $FULL_IMPORT_URL"

                echo "Executing: curl -X PUT -H \"Authorization: Bearer ${ACCESS_TOKEN}\" -F file=@\"${name}.csv\" -F type=application/octet-stream \"$FULL_IMPORT_URL\""
                curl -X PUT -H "Authorization: Bearer ${ACCESS_TOKEN}" -F file=@"${name}.csv" -F type=application/octet-stream "$FULL_IMPORT_URL"
            else

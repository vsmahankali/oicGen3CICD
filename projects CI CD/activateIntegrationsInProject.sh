...
wait_for_activation() {
	...
	echo
    echo "Waiting for activation of ${INTEGRATION_ID}..."
    ...
    "${OIC_URL}/ic/api/integration/v1/projects/${PROJECT_CODE}/integrations/${ENCODED_ID}/activationStatus?integrationInstance=${INTEGRATION_INSTANCE}")
        ...
        case "${STATUS}" in
            ...
            ACTIVATED)
                echo "✓ Activation completed for ${INTEGRATION_ID}"
                return 0
                ;;

            CONFIGURED|ACTIVATION_INPROGRESS|FAILEDACTIVATION)
                sleep ${WAIT_TIME}
...
# Input CSV
read -p "Enter project integration list to be activated file name: " INPUT_FILE_NAME
...
do
    ...
    echo "Integration to activate: ${INTEGRATION_CODE}"
...
do
    ...
    if [[ "${STATUS}" == "CONFIGURED" ]] && [[ "${CURRENT_INTEGRATION_CODE}" == "${INTEGRATION_CODE}" ]]; then
        ...
        REQUEST_BODY='{"status":"ACTIVATED"}'

        echo "Activating ${INTEGRATION_ID}"
        ...
        ACTIVATION_RESPONSE=$(curl -X POST \
          -H "Authorization: Bearer ${ACCESS_TOKEN}" \
		...
		ERROR_CODE=$(echo "${ACTIVATION_RESPONSE}" | jq -r '.errorCode // empty')
		...
		if [[ -n "${ERROR_CODE}" ]]; then
   			 ERROR_STATUS=$(echo "${ACTIVATION_RESPONSE}" | jq -r '.status')
   			 ERROR_MESSAGE=$(echo "${ACTIVATION_RESPONSE}" | jq -r '.title')
		 ...
		 # Wait until activation completes
           wait_for_activation "${PROJECT_CODE}" "${INTEGRATION_ID}" "${ACCESS_TOKEN}"

          if [ $? -eq 0 ]; then
             echo "Proceeding to next integration..."
          else
             echo "Activation timeout/failure for ${INTEGRATION_ID}"
          fi
		fi          
    fi
done
